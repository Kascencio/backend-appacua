...
```bash
curl http://localhost:3100/api/usuarios \
  -H "Authorization: Bearer TU_JWT"
```

Lecturas de sensor instalado:
```bash
curl "http://localhost:3100/api/lecturas?sensorInstaladoId=1&from=2026-02-20T00:00:00Z&to=2026-02-26T23:59:59Z&limit=500" \
  -H "Authorization: Bearer TU_JWT"
```

Promedios por bucket (analitica):
```bash
curl "http://localhost:3100/api/promedios?sensorInstaladoId=1&bucketMinutes=15&from=2026-02-20T00:00:00Z&to=2026-02-26T23:59:59Z" \
  -H "Authorization: Bearer TU_JWT"
```
...
Canales:
- Lecturas:
  - `ws://HOST:3100/ws/lecturas?sensorInstaladoId=<id>`
  - `ws://HOST:3100/ws/lecturas?instalacionId=<id>`
- Notificaciones:
  - `ws://HOST:3100/ws/notificaciones`
